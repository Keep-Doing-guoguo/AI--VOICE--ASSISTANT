...
独立设计与开发一个支持实时语音识别、连续听写、多轮上下文理解及流式回答输出的智能语音助手系统。

系统采用麦克风实时录音，通过 DashScope gummy-realtime-v1 进行语音识别（ASR），识别完成的语句自动送入大模型（DeepSeek-V2.5）进行推理，并通过 SSE/WebSocket 将生成内容逐字实时返回到页面。
